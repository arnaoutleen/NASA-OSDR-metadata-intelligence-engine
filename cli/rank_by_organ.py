...
def rank_matching_samples(records: List[SampleRecord]) -> pd.DataFrame:
    scorer = SampleInformativenessScorer()
    return scorer.score(records)


def summarize_matching_assays(records: List[SampleRecord], organ_label: str, resolver: MissionResolver) -> pd.DataFrame:
    rows = []
    for record in records:
        assay_types = sorted({assay for assay in record.measurement_types if assay})
        if not assay_types:
            assay_types = [""]

        for assay in assay_types:
            rows.append({
                "mission": resolver.get_mission_for_osd(record.osd) or f"organ_query:{organ_label}",
                "OSD": record.osd,
                "organ": record.material_type,
                "assay_type": assay,
                "source_name": record.source_name,
                "sample_name": record.sample_name,
                "data_files_count": len(record.data_files),
            })
    ...
    grouped = (
        ...
        .reset_index()
        .sort_values(
            by=["mission", "OSD", "num_matching_samples", "num_matching_mice", "assay_type"],
    ...
    grouped["informativeness_rank"] = (
        grouped.groupby(["mission", "OSD"])["num_matching_samples"]
...
Examples:
    ...
    python -m cli.rank_by_organ --organ liver --sample-output outputs/rankings/liver_samples.csv
    python -m cli.rank_by_organ --list-organs
""",
    )
    ...
    parser.add_argument("--sample-output", type=Path, help="Optional CSV path for the organ-filtered sample ranking table")
    parser.add_argument("--assay-output", type=Path, help="Optional CSV path for the organ-filtered assay summary table")
...
def main() -> int:
    ...
    sample_df = rank_matching_samples(matched_records)
    assay_df = summarize_matching_assays(matched_records, organ_label=organ, resolver=resolver)
    ...
    sample_output_path = args.sample_output or (defaults["output_dir"] / "rankings" / f"sample_ranking_{label}.csv")
    assay_output_path = args.assay_output or (defaults["output_dir"] / "rankings" / f"assay_ranking_{label}.csv")
    ...
    assay_output_path.parent.mkdir(parents=True, exist_ok=True)
    assay_df.to_csv(assay_output_path, index=False)
    ...
    print(f"Ranked samples: {len(sample_df)}")
    print(f"Assay summary rows: {len(assay_df)}")
    print(f"Mouse ranking output written to: {output_path}")
    print(f"Sample ranking output written to: {sample_output_path}")
    print(f"Assay summary output written to: {assay_output_path}")
    if args.studies_output:
